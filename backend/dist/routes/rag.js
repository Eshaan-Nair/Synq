...
router.post("/retrieve", async (req, res) => {
    ...
    try {
        logger_1.logger.info(`RAG retrieve (topN=${topN}): "${String(prompt).slice(0, 60)}..." for session ${sessionId}`);
        const chunks = await (0, chroma_1.retrieveRelevantChunks)(prompt, sessionId, topN);
        if (chunks.length === 0) {
            logger_1.logger.info("RAG: no chunks above threshold — skipping injection");
        ...
        const contextBlock = chunks
            ...
            .join("\n\n---\n\n");
        logger_1.logger.success(`RAG: ${chunks.length} chunk(s) found — scores: ${chunks.map(c => c.score.toFixed(2)).join(", ")}`);
        res.json({
            found: true,
            chunks,
            contextBlock,
        ...
        logger_1.logger.error("RAG retrieve error:", err);
